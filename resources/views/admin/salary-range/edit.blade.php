...
@section('panel')
    <div class="row mb-none-30">
        <div class="col-lg-12 mb-30">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.salaryrange.update') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                                    ...
                                    <select class="form-control" name="stream_id" id="stream" required>
                                        ...
                                        @foreach ($streams as $stream)
                                            <option value="{{ $stream->id }}"
                                                {{ $stream->id == $edit->stream_id ? 'selected' : '' }}>
                                                {{ $stream->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select class="form-control" name="subcategory_id" id="subcategory" required>
                                        ...
                                        @foreach ($subcategories as $subcategory)
                                            <option value="{{ $subcategory->id }}"
                                                {{ $subcategory->id == $edit->subcategory_id ? 'selected' : '' }}>
