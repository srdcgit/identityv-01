...
@section('panel')
    <div class="row mb-none-30">
        <div class="col-lg-12 mb-30">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.category2.update') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <select name="category_id" class="form-control">
                                        ...
                                        @foreach ($category as $data)
                                            <option value="{{ $data->id }}" {{ ($data->id == $edit->category_id)? 'selected' : '' }}>{{ $data->title }}</option>
                                        @endforeach
                                    </select>
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.category2.list') }}" class="btn btn-sm btn--primary box--shadow1 text--small"><i
